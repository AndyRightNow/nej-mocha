...
describe('src/node/util/normalize-user-config', () => {
  describe('Helper functions', () => {
    ...
    describe('normalizeNejPathAliases', () => {
      describe('If nothing is given', () => {
        it('should set pro to default and append a slash to it', () => {
          ...
          expect(userConfig.nejPathAliases.pro).to.equal(config.CONSTANT.DEFAULT_NEJ_PRO + '/')
        })
      })
    })

    describe('normalizeTestRunnerPort', () => {
      describe('If nothing is given', () => {
        it('should set it to default', () => {
          helpers.normalizeTestRunnerPort(userConfig)

          expect(userConfig.testRunnerPort).to.equal(config.CONSTANT.DEFAULT_PORT)
        })
      })
    })
    ...
    describe('normalizeProxy', () => {
      describe('If nothing is given', () => {
        it('should set it to the testRunnerPort and localhost', () => {
          helpers.normalizeProxy(userConfig)

          expect(userConfig.proxy.port).to.equal(config.CONSTANT.DEFAULT_PORT)
    ...
    describe('normalizeScriptsToInject', () => {
      describe('If paths are relative paths', () => {
        it('should normalize slashes', () => {
          userConfig.normalizeScriptsToInject = [
            './test',
            './..\\test/test'
          ]
